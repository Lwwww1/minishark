 ...
 * protocol.c — 协议解析实现
 *
 * 链路层 (Ethernet) 与网络层 (IPv4/IPv6) 的基础解析入口。
 * Day 2 范围：parse_eth() / parse_ipv4() / parse_ipv6()。
 * Day 3 起会再补充 parse_tcp() / parse_udp() / parse_icmp() 等上层协议，
 * 并由 parse_ipv4 / parse_ipv6 按 proto 字段分发。
 *
...
int parse_ipv4(const uint8_t *pkt, size_t len)
{
    ...
    /* Day 3 起：依据 proto 分发到 parse_tcp / parse_udp / parse_icmp */
    /* 这里先保持 Day 2 范围：不递归向上层，仅打印链路层 + 网络层信息。 */
...
int parse_ipv6(const uint8_t *pkt, size_t len)
{
    ...
    /* Day 3+：处理扩展头链，然后按 next 分发到上层 parser */
    return 0;
}
